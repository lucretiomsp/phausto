...
class mydsp : public dsp {
 ...
 public:
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow3 = std::max<float>(1.00000001e-07f, std::fabs(float(fEntry2)));
		...
		for (int i = 0; (i < count); i = (i + 1)) {
			...
			float fTemp2 = (fSlow4 + (fRec5[1] + -1.0f));
			int iTemp3 = (fTemp2 < 0.0f);
			float fTemp4 = (fSlow4 + fRec5[1]);
			fRec5[0] = (iTemp3 ? fTemp4 : fTemp2);
			float fRec6 = (iTemp3 ? fTemp4 : (fSlow4 + (fRec5[1] + (fSlow5 * fTemp2))));
			float fTemp5 = ((fRec3[0] * fRec4[0]) * ((2.0f * fRec6) + -1.0f));
			fVec0[0] = fTemp5;
